...
bool Hub::handleMessage(const message::Message &recv, Hub::socket_ptr sock, const buffer *payload)
{
    ...
    case message::KILL: {
        ...
        if (handlePriv(kill))
            return true; //else fall through to handleDefaultMessage
    }
    default:
        break;
    }

    handleDefaultMessage(msg, payload, senderType);
...
void Hub::handleDefaultMessage(message::Message &msg, const buffer *payload, message::Identify::Identity senderType)
{
