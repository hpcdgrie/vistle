...
struct HubPythonStateAccessor: public vistle::PythonStateAccessor {
    ...
    bool sendMessage(const vistle::message::BufferEnvelope &message) override
    {
        bool ret = vistle::Hub::the().handleMessage(message, nullptr, vistle::message::Identify::SCRIPT);
        assert(ret);
        if (!ret) {
            std::cerr << "Python: failed to send message " << message.header() << std::endl;
        }
        return ret;
    }
};
