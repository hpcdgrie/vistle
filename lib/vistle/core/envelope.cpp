...
buffer Envelope::copyPayload() const
{
    ...
    return buffer(payload(), payload() + payloadSize());
}

const char *Envelope::payload() const
{
    if (m_header.payloadSize() == 0) {
        return nullptr;
    }
...
const std::shared_ptr<vistle::buffer> &BufferEnvelope::accessPayload() const
{
    return m_payload;
}

std::shared_ptr<vistle::buffer> &BufferEnvelope::accessPayload()
{
    return m_payload;
}
