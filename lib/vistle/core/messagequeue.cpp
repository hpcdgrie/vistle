...
{
    assert(msg.externalPayloadSize() == 0 || msg.shmPayload());
    std::unique_lock<std::mutex> guard(m_mutex);
    msg.ref();
    if (priority == 0) {
        m_queue.emplace_back(msg.message());
    } else {
        m_prioQueues[priority].emplace_back(msg.message());
    }
    guard.unlock();
    return progress();
}
...
bool MessageQueue::receive(ShmEnvelope &msg, unsigned int *ppriority)
{
...
#else
    while (!m_mq.timed_receive(&msg.message(), message::Message::MESSAGE_SIZE, recvSize, priority,
    ...
    if (recvSize != message::Message::MESSAGE_SIZE)
        return false;
    if (msg.message().payloadSize() > 0) {
        msg.updateMessage();
        if (!msg.hasInternalPayload()) {
...
bool MessageQueue::tryReceive(ShmEnvelope &msg, unsigned int minPrio, unsigned int *ppriority)
{
    ...
    size_t recvSize = 0;
    unsigned priority = 0;
    bool result = m_mq.try_receive(&msg.message(), message::Message::MESSAGE_SIZE, recvSize, priority);
    if (result) {
