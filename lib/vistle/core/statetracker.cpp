...
void appendMessage(StateTracker::VistleState &v, const MessagePayload &msg)
{
    v.emplace_back(msg);
}
...
StateTracker::VistleState StateTracker::getState() const
{
    ...
    for (const auto &keymod: availableModules()) {
        keymod.second.send([&state](const MessagePayload &payload) {
