...
bool Module::sendMessage(const vistle::message::Envelope &message) const
{
...
#else
    newMessage->message().setPayloadRawSize(message.payloadSize());
#endif
    if (auto shmMsg = dynamic_cast<const vistle::ShmEnvelope *>(newMessage.get())) {
        ...
        if (message.externalPayloadSize() > 0) {
            sendMessageQueue->send(ShmEnvelope(newMessage->message(), newMessage->copyPayload()));
        } else {
            sendMessageQueue->send((ShmEnvelope(newMessage->message())));
        }
    }
