...
bool Module::getNextMessage(ShmEnvelope &msg, bool block, unsigned int minPrio)
{
    if (!receiveMessageQueue)
        return false;

    bool recv = receiveMessageQueue->tryReceive(msg);
    do {
        if (recv) {
            if (!messageBacklog.empty()) {
                auto &front = messageBacklog.front();
                if (msg.header().priority() <= front.header().priority()) {
                    // keep buffered messages sorted according to priority
                    std::swap(msg, front);
                    auto it = messageBacklog.begin(), next = it + 1;
                    while (next != messageBacklog.end()) {
                        if (it->header().priority() <= next->header().priority())
                            std::swap(*it, *next);
                        it = next;
                        ++next;
                    }
            ...
            if (msg.header().priority() >= minPrio) {
                return true;
            }

            messageBacklog.push_front(msg);
        } else if (!messageBacklog.empty()) {
            const auto &front = messageBacklog.front();
            if (front.header().priority() >= minPrio) {
                msg = front;
        ...
        if (block) {
            receiveMessageQueue->receive(msg);
            recv = true;
        }
    } while (block);
...
bool Module::dispatch(bool block, bool *messageReceived, unsigned int minPrio)
{
    ...
    try {
        ...
        ShmEnvelope pl;
        if (!getNextMessage(pl, block, minPrio)) {
            if (messageReceived)
        ...
        if (syncMessageProcessing()) {
            ...
            do {
                ...
                again &= handleMessage(pl);
                if (!again) {
                    CERR << "collective, quitting after " << buf << std::endl;
                }

                if (allsync && !sync) {
                    getNextMessage(pl, true, minPrio);
                    buf = pl.header();
                }
...
void Module::sendParameterMessage(const vistle::message::Envelope &message) const
{
    sendMessage(message);
}

bool Module::sendMessage(const vistle::message::Envelope &message) const
{
    if (!sendMessageQueue)
        return false;

    auto &msg = message.header();
    if (msg.type() != message::SENDTEXT && (m_traceMessages == message::ANY || m_traceMessages == msg.type())) {
        CERR << "SEND: " << msg << std::endl;
    }

    if (!(rank() == 0 || message::Router::the().toRank0(msg)))
        return true;

    auto newMessage = message.clone();
#ifdef MODULE_THREAD
    newMessage->header().setRank(rank());
    newMessage->header().setSenderId(id());
#else
    newMessage->header().setPayloadRawSize(message.payloadSize());
#endif
    if (auto shmMsg = dynamic_cast<const vistle::ShmEnvelope *>(newMessage.get())) {
        sendMessageQueue->send(*shmMsg);
    } else {
        if (message.externalPayloadSize() > 0) {
            sendMessageQueue->send(ShmEnvelope(newMessage->header(), {newMessage->begin(), newMessage->end()}));
        } else {
            sendMessageQueue->send((ShmEnvelope(newMessage->header())));
        }
    }
...
bool Module::handleMessage(const vistle::ShmEnvelope &vistleMsg)
{
    using namespace vistle::message;

    const auto &message = vistleMsg.header();
    size_t payloadSize = message.payloadSize();
    const char *payload = vistleMsg.data();


    if (payload)
        m_stateTracker->handle(message, payload, payloadSize);
    else
...
{
    ShmEnvelope msg;
    while (!m_executeAfterCancelFound && receiveMessageQueue && receiveMessageQueue->tryReceive(msg)) {
        messageBacklog.push_back(msg);
        switch (msg.header().type()) {
        case message::CANCELEXECUTE: {
            const auto &cancel = msg.as<message::CancelExecute>();
