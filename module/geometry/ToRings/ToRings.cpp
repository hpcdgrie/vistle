...
ToRings::ToRings(const std::string &name, int moduleID, mpi::communicator comm): Module(name, moduleID, comm)
{
    ...
    m_stretch = addFloatParameter("spline radius", "radius of the rings", 1);
    m_numXSegments =
...
std::vector<Vector3> ToRings::toTorus(const std::vector<Vector3> &points, Index numTorusSegments,
                                      Index numDiameterSegments)
{
    auto middle = center(points);
    if (m_radius->getValue() < 0)
        setFloatParameter(m_radius->getName(), (middle - points[0]).norm());
...
std::vector<Vector3> ToRings::toTorusCircle(const std::vector<Vector3> &points, const Vector3 &middle,
...
std::vector<Vector3> ToRings::toTorusSpline(const std::vector<Vector3> &points, const Vector3 &middle,
                                            Index numTorusSegments, Index numDiameterSegments)
{
    auto ps = points.size();
    ...
    for (int i = 0; i < v1.size(); i++) {
        auto currentPoint = middle + (v1[i] - middle) * m_stretch->getValue();
